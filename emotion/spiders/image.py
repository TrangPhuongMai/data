...
class ImageSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for emotion in emotions:
            ...
            yield Request(
                url=self.start_urls[0] + emotion + '?orientation=horizontal&image_type=illustration&color=green',
    ...
    def parse_stock(self, response):
        ...
        """
        yield SplashRequest(url=response.url, callback=self.parse_png, endpoint='execute',
    ...
    def parse_adfBypass(self, response):
        ...
        yield Request(url=url, callback=self.parse_img, meta={'url': url})
    ...
    def parse_img(self, response):
        ...
        # l.add_css('image_urls','body > div.container > div.row > div > a')
        print('--------------------------------------------------')
        # print(response.xpath('//div[@class="row"]//img').attrib['src'])

        # print(response.xpath('body > div.container > div.row > div > a').extract())
        img_url = response.xpath('//div[@class="row"]//img').attrib['src']
        img_url = 'https://findandfound.ga' +  img_url
        print(img_url)
